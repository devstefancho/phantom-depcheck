...
// 커맨드 라인 인자에서 srcPath를 파라미터로 받기 (기본값은 'src')
const srcPath = path.join(process.cwd(), process.argv[2] || "src");
...
function findImportsInFile(filePath) {
  ...
  const customFilteredPackages = ["react", "react-dom", "public"];
  // node_modules 패키지만 필터링 (상대 경로, Path Alias, submodules 제외)
  return Array.from(imports)
    .map((importPath) => importPath.split("/")[0])
    .filter((pkg) => customFilteredPackages.indexOf(pkg) === -1)
    .filter((pkg) => ![".", "..", "/"].some((char) => pkg.startsWith(char)))
    .filter((pkg) => {
      if (pkg.startsWith("@")) {
        return pkg === "@types";
      }
